...
import { Upload, Download, AlertCircle, CheckCircle, Calendar, BarChart, Users } from 'lucide-react';
...
const HRDataUpload: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'salary' | 'attendance'>('attendance');
  ...
  const handleEmployeeConfirmation = async (missingEmployees: any[]) => {
    try {
      ...
      logger.info( '✅ Created employees:', createResult);
      ...
      logger.error('❌ Error creating employees:', error);
      setAttendanceUploadMessage('Failed to create missing employees. Please try again.');
      setAttendanceUploadStatus('error');
    }
  };
  ...
  const performUpload = async (uploadData: { file: File; month: string; year: string; type: 'salary' | 'attendance' }) => {
    ...
    // Salary upload functionality - COMMENTED OUT
    if (type === 'salary') {
      /*
      setUploading(true);
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.info( '📡 Dispatching dataUploaded and refreshEmployeeData events');
  ...
  const allMonthOptions: DropdownOption[] = [
  ...
  // Limit months up to current month when selected year is current year
  const monthOptions: DropdownOption[] = useMemo(() => {
    if (attendanceYear === currentYear.toString()) {
      // Keep the placeholder and slice the rest up to current month
      const placeholder = allMonthOptions[0];
      const monthsUpToCurrent = allMonthOptions.slice(1, 1 + (currentMonthIndex + 1));
      return [placeholder, ...monthsUpToCurrent];
    }
    return allMonthOptions;
  }, [attendanceYear, currentYear, currentMonthIndex]);

  // If currently selected month becomes invalid for current year, clear it
  useEffect(() => {
    if (!attendanceMonth) return;
    if (attendanceYear !== currentYear.toString()) return;

    const monthIndexMap: { [key: string]: number } = {
      JAN: 0, FEB: 1, MAR: 2, APR: 3, MAY: 4, JUN: 5,
      JUL: 6, AUG: 7, SEP: 8, OCT: 9, NOV: 10, DEC: 11
    };
    const selectedIndex = monthIndexMap[attendanceMonth];
    if (selectedIndex !== undefined && selectedIndex > currentMonthIndex) {
      setAttendanceMonth('');
    }
  }, [attendanceYear, attendanceMonth, currentYear, currentMonthIndex]);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg p-6 shadow-sm">
        ...
        {/* Salary Upload Tab - COMMENTED OUT */}
        {false && activeTab === 'salary' && (
          <div>
            {/* ... salary upload UI ... */}
          </div>
        )}
